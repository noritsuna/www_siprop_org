...
これに加えて、特定のルールによるテキスト整形を行うことができます。以下では、プラグインについてはよく使われるものだけを簡略に説明しています。プラグインのオプションなど詳細な説明については [PukiWiki/1.4/Manual/Plugin](PukiWiki/1.4/Manual/Plugin.md) を参照してください。
...
| <span style="color: crimson">左寄せ</span> | センタリング | 右寄せ <!-- BGCOLOR(yellow) --> |
| 右寄せ | 左寄せ | センタリング |
